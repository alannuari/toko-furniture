...
<section class="container mx-auto">
    ...
    <div class="flex-1 px-4 md:p-6">
        ...
        <form action="{{ route('cart-add', $product->id) }}" method="POST">
            @csrf
            <button type="submit" class="transition-all duration-200 bg-pink-400 text-black focus:bg-black focus:text-pink-400 rounded-full px-8 py-3 mt-4 inline-flex">
        ...
        @foreach ($recommendations as $recommendation)
            <div class="px-3 flex-none" style="width: 320px">
                <div class="rounded-xl p-4 pb-8 relative bg-white">
                    ...
                    <a href="{{ route('details', $recommendation->slug) }}" class="stretched-link"></a>
                </div>
            </div>
        @endforeach
        </div>
